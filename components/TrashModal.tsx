...
export default function TrashModal({
  ...
  const handleRestore = async (id: string) => {
    setActionId(id);
    try {
      const res = await fetch(`/api/trash/${trashType}/${id}`, { 
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ restore: true, deletedAt: null }) 
      });
      
      if (!res.ok) {
        const errorData = await res.json().catch(() => ({}));
        alert(errorData.message || 'Geri yükleme işlemi başarısız oldu.');
        return; 
      }
