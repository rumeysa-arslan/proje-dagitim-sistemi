...
interface NavbarProps {
  ...
  currentPlan?: string; // Plan bilgisi buraya eklendi
}

export default function Navbar({ user, onLogout, onChangePassword, currentPlan }: NavbarProps) {
  ...
  return (
    <header className="bg-white border-b border-gray-100 shadow-2xs mb-6">
      <div className="max-w-6xl mx-auto px-6 py-4 flex justify-between items-center">
        ...
        {/* ⚡ Dinamik Çevirili Logo ve Plan Etiketi */}
        <div className="flex items-center gap-3 font-bold text-gray-800 text-base">
          <span className="bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent">
            {t('appName')}
          </span>
          
          {/* PLAN ROZETİ BURAYA EKLENDİ */}
          {currentPlan && (
            <span className={`px-2.5 py-0.5 text-[10px] font-extrabold tracking-wider uppercase rounded-full border ${
