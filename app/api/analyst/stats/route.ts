...
export async function GET(request: Request) {
  try {
    ...
    const pms = await prisma.user.findMany({
      where: { tenantId: user.tenantId, role: 'PM', deletedAt: null, isActive: true },
      select: { id: true, name: true, email: true },
    });

    const developers = await prisma.user.findMany({
      where: {tenantId: user.tenantId,  role: 'DEVELOPER', deletedAt: null, isActive: true },
      select: { id: true, name: true, email: true },
    });
    ...
    if (type === 'PM') {
      ...
      const projects = await prisma.project.findMany({
        where: { tenantId: user.tenantId, createdById: pmId, deletedAt: null },
        include: {
          tasks: {
