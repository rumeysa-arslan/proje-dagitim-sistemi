...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedTenant) {
      setMessage({ type: 'error', text: 'Lütfen başvurmak istediğiniz şirketi seçin.' });
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.message || 'Başvuru alınamadı.');
      }

      setMessage({ type: 'success', text: data.message || 'Başvuru başarıyla alındı.' });
      setName('');
      setEmail('');
      setSelectedTenant('');
      setRole('DEVELOPER');
      ...
      <div className="bg-white dark:bg-zinc-900 w-full max-w-md p-8 rounded-2xl shadow-xl border border-gray-100 dark:border-zinc-800">
        <div className="text-center mb-6">
          ...
          <p className="text-xs text-gray-500 mt-1">
            Bilgilerinizi doldurun, admin onayından sonra şifre linkiniz e-postanıza gelsin.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {/* ŞİRKET SEÇİMİ */}
          <div>
            <label className="block text-xs font-semibold text-gray-700 dark:text-zinc-300 mb-1">Şirket</label>
            <select
              required
            ...
            >
              <option value="" disabled>Lütfen bir şirket seçin</option>
              {tenants.map((tenant) => (
